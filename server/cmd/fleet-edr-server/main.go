...
func registerSessionRoutes(mux *http.ServeMux, d muxDeps) {
	...
	for _, p := range []string{
		...
		"GET /api/session",
		// Phase 5 break-glass reauth ceremony. The handlers are
		// mounted on apiMux via identityCtx.RegisterAuthedRoutes, but
		// the outer router needs each path enumerated here so the
		// session-protected wrapper actually serves them — otherwise
		// requests fall through to the `/` catchall and 302 → /ui/,
		// silently breaking the destructive-action reauth path.
