...
// Preloaded asset bytes. Reading from embed.FS on every request allocates a fresh []byte per call (the embed package has no shared
// backing buffer for consumers), so we pay that cost once at init.
var (
...
// setSecurityHeaders applies the always-on hardening: nosniff on every asset handler prevents a browser from interpreting, say,
// a spec file shipped with the wrong Content-Type as an executable script.
...
func serveSpec(w http.ResponseWriter, r *http.Request) {
	...
	// Spec URL in the HTML is also a bare path; revalidate-every-load is the simplest way to guarantee no stale spec after a server
	// upgrade. 304 responses carry no body, so the cost is one round-trip.
	w.Header().Set(headerCacheControl, cacheControlNoCache)
