...
type Rule interface {
	...
	// (the rule resolved to monitor mode) rides the batch tally instead, which a retryable error drops so a replayed batch is not
	// counted twice; since issue #893 the queue carries that tally across the batch's own retries and hands it to whichever
	// attempt withdraws the batch, so a set-aside batch still contributes its count.
	//
...
// `Source=admin`, `CreatedBy=<session user>` and timestamps. Severity defaults to medium and Enforcement to PROTECT when blank:
// a caller that says nothing about enforcement gets a rule that blocks, as every rule did before DETECT had a meaning.
