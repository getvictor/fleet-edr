...
type Event struct {
	...
	// Platform is the operating system that produced the event, one of the Platform* constants. It is optional on the wire: an agent
	// predating the platform-aware contract omits it, and intake normalizes an empty value to PlatformDarwin (see NormalizePlatform).
	// omitempty keeps the wire byte-identical for a legacy agent that never sets it.
