...
--
-- Two columns rather than one, because one bound cannot tell the two failure shapes apart. attempts alone would set aside anything
-- that failed for ten seconds; time-since-first-failure alone would set aside a batch that failed once and then sat because the
-- host went quiet. A batch is set aside only when BOTH bounds are exceeded; the thresholds and their reasoning live next to the
-- Nack that applies them.
...
-- but the sweep is fleet-wide and orders by when the batch first failed.
-- +goose StatementBegin
CREATE INDEX idx_event_queue_set_aside ON event_queue (processed, first_failed_at_ns);
-- +goose StatementEnd
...
-- +goose StatementBegin
ALTER TABLE event_queue DROP COLUMN attempts, DROP COLUMN first_failed_at_ns, ALGORITHM=INPLACE, LOCK=NONE;
-- +goose StatementEnd
