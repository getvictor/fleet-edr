...
// WriteSecretFile is a tiny helper for tests and docker-compose fixtures: it writes the given value to a tempfile with 0600 perms
// and returns the path. Kept in the non-test file so integration tests that live outside the server/config package can reach it.
// Uses filepath.Join so trailing slashes on dir and OS path separators are handled correctly.
