...
type MetricsRecorder interface {
	...
	// DetectionMaterializationRetry is called by the processor each time it re-queues (nacks) a batch because rule evaluation hit an
	// event whose subject or flow process was not materialized yet (a transient ordering race; issue #631). It is the bounded,
	// observable replacement for warn-logging every retry: the processor logs the retry at DEBUG and increments this counter, so a
	// sustained materialization-miss backlog stays detectable without flooding the logs or the OTLP export.
