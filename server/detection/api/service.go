...
type MetricsRecorder interface {
	...
	// the accompanying log line carries on a consequence attribute: a gap in the process graph at the builder stage, and at the
	// detection stage an intact graph with detection unfinished, so alerts those events would have raised may be missing.
