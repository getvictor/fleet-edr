...
// failed read, where the next event's read reaches the same unavailable dependency. That is the whole divergence (issue #844). It
// is logged the same way, quietly and per attempt, and it does NOT stop the batch's other rules; both are deliberate and the
// sentinel's own comment says why.
//
// The message no longer says "unavailable" itself, because the sentinel it wraps now does: repeating it read as two separate
// claims about the same failure.
