...
// ctxKey is unexported so callers must go through the With* / *FromContext
// helpers below. The actual middleware in server/endpoint/internal/
// middleware uses these helpers too -- production code path and test
// path both write through the same setter.
type ctxKey int

const ctxKeyHostID ctxKey = iota + 1

// HostIDFromContext returns the host_id pinned by the HostToken
// middleware. The second return is false when the context was not
// wrapped (caller bypassed middleware -- a programming error in
// production; tests should use WithHostIDForTest).
...
// WithHostIDForTest is a backward-compat alias for WithHostID. Existing
