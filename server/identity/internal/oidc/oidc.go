...
// clockSkewTolerance is the ±2-minute window the spec calls for on
// ID-token expiry / not-before checks. go-oidc/v3's verifier doesn't
// expose a direct SkewTolerance knob; the implementation accepts
// tokens whose Expiry has not yet passed against the verifier's Now
// func. We honour the spec by passing a Now that subtracts the
// tolerance for expiry checks (so a token "expired 90s ago" still
// verifies). IAT/NBF checks consume the same Now so a token issued
// in the future by less than the tolerance also verifies.
const clockSkewTolerance = 2 * time.Minute

// Options bundles the per-deployment knobs the OIDC client needs at
// construction time. Issuer is the discovery URL (the well-known doc
// lives at <Issuer>/.well-known/openid-configuration). Scopes default
// to [openid, email, profile] so the verifier sees the claims it
...
// Client wraps the go-oidc Provider + Verifier + the oauth2 Config
// they share. Construct once at boot via New; the Provider's JWKS
// cache lives for the process lifetime and refreshes lazily on
// verifier failure.
type Client struct {
...
// New runs OIDC discovery against opts.Issuer and prepares a verifier
// + oauth2 config. ctx is used for the discovery HTTP call; cancelling
// it before New returns aborts setup. Returns an error on issuer
// reachability failure or malformed discovery doc; cmd/main maps this
// to a refuse-to-start error.
...
// GenerateFlowSecrets returns a fresh (state, nonce, code_verifier,
// code_challenge) tuple. state + nonce are 32-byte URL-safe random
// strings; the verifier is also 32 bytes URL-safe; the challenge is
// the S256 hash of the verifier per RFC 7636. Callers persist
// (state, nonce, verifier) in the signed state cookie and echo
// (state, challenge) in the AuthURL.
...
// randomURLSafe returns a URL-safe base64-encoded string of n random
// bytes. Output length is approximately ceil(n * 4 / 3) before
