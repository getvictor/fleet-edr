...
// StateCookieName is the HTTP cookie that carries the per-flow
// secrets the callback needs to verify. HttpOnly + Secure (in TLS
// deployments) + SameSite=Lax. Lax is REQUIRED — Strict would block
// the IdP's cross-site GET-redirect that lands the callback.
const StateCookieName = "edr_oidc_state"

// stateClaim is the JSON payload signed into the state cookie. State
// is the OAuth2 state value (echoed in the AuthURL and the callback
// query string); Nonce is the OIDC nonce verified against the ID
// token; CodeVerifier is the PKCE secret; Redirect is where to send
// the operator after a successful login. IssuedAt enforces the cookie
// TTL on the verifier side so a stale cookie is rejected even if the
...
// ErrInvalidStateCookie groups every "you can't trust this cookie"
// failure into one error type so the handler can map it to a single
// 400 wire shape regardless of which check tripped. Specific reasons
// are surfaced via Wrapped().
var ErrInvalidStateCookie = errors.New("oidc: state cookie invalid")

// EncodeStateClaim serializes c into "<base64url(json)>.<base64url(sig)>"
// where sig = HMAC-SHA256(payload64, key). The cookie is not
// confidential — payload64 is plain JSON over base64 so anyone
// holding the cookie can read state + nonce + code_verifier — but it
// IS authenticated against the signing key. An attacker who replays
// the cookie at a different IdP or with a swapped state value gets a
...
// DecodeStateClaim verifies the cookie's signature with key, checks
// the IssuedAt against (now, ttl), and returns the decoded payload.
// Any failure (malformed, bad signature, expired) returns
// ErrInvalidStateCookie wrapped with a more specific reason for the
// handler's slog line.
