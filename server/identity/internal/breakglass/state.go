...
func DecodeChallengeState(signingKey []byte, raw string) (webauthn.SessionData, error) {
	...
	if err != nil {
		return webauthn.SessionData{}, errors.Join(ErrChallengeStateInvalid,
			fmt.Errorf("signature decode: %w", err))
	}
	payload, err := base64.RawURLEncoding.DecodeString(string(parts[1]))
	if err != nil {
		return webauthn.SessionData{}, errors.Join(ErrChallengeStateInvalid,
			fmt.Errorf("payload decode: %w", err))
	}
	mac := hmac.New(sha256.New, signingKey)
	mac.Write(payload)
	want := mac.Sum(nil)
