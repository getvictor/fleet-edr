...
func newAsyncEngine(t *testing.T, rate float64) (*authz.Engine, *recordingAudit, *recordingAsync) {
	...
	e, err := authz.New(t.Context(), sync, nil, false, authz.Options{
		AsyncRead:        asyncW,
		ReadSamplingRate: rate,
	})
	require.NoError(t, err, "construct engine")
	return e, sync, asyncW
}
...
func TestAllow_ReadAllow_RateZero_DropsEverywhere(t *testing.T) {
	e, sync, async := newAsyncEngine(t, 0.0)
	...
	assert.Empty(t, sync.snapshot(), "rate=0.0 must NOT write to the sync recorder")
	assert.Empty(t, async.snapshot(), "rate=0.0 must NOT submit to the async writer")
}
...
func TestAllow_ReadAllow_RateOne_RoutesAsync(t *testing.T) {
	e, sync, async := newAsyncEngine(t, 1.0)
	actor := actorWithRoles(1, "default", tenantBinding("auditor", "default"))
	ctx := api.WithActor(t.Context(), actor)
	d, err := e.Allow(ctx, api.ActionAlertRead, api.Resource{TenantID: "default", Type: "alert"})
	require.NoError(t, err)
	assert.True(t, d.Allow)
	assert.Empty(t, sync.snapshot(), "rate=1.0 read-allow must NOT write sync; the async writer is the durable sink")
	require.Len(t, async.snapshot(), 1)
	assert.Equal(t, api.AuditAction("authz.alert.read"), async.snapshot()[0].Action)
}
...
func TestAllow_ReadAllow_BreakGlass_AlwaysSync(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, d.Allow)
	require.Len(t, sync.snapshot(), 1, "break-glass reads MUST write sync")
	assert.Empty(t, async.snapshot(), "break-glass reads must NOT take the async path")
}
...
func TestAllow_AuditRead_AlwaysSync(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, d.Allow)
	require.Len(t, sync.snapshot(), 1, "audit.read MUST always emit the audit-of-audit row")
	assert.Equal(t, api.AuditAction("authz.audit.read"), sync.snapshot()[0].Action)
	assert.Empty(t, async.snapshot(), "audit.read does not take the async path")
}
...
func TestAllow_ReadDeny_AlwaysSync(t *testing.T) {
	...
	require.Len(t, sync.snapshot(), 1, "deny MUST emit sync regardless of read sampling")
	assert.Empty(t, async.snapshot())
}
...
func TestAllow_WriteAllow_AlwaysSync(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, d.Allow)
	require.Len(t, sync.snapshot(), 1, "write actions MUST write sync at every rate")
	assert.Empty(t, async.snapshot())
}
...
func TestAllow_ReadAllow_AsyncDrop_FallsBackToSync(t *testing.T) {
	...
	// recorder instead.
	require.Len(t, sync.snapshot(), 1, "drop on full async queue must fall back to sync")
	assert.Empty(t, async.snapshot(), "no event made it past the dropping Submit")
}
