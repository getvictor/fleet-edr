...
// The chokepoint reads it through Service.LoadActor so the actor's AuthMethod
// field reflects ground truth instead of a hardcoded default. IdentityID FKs
// into the identities table; nullable so legacy local-password rows and tests
// that don't track identities can stay valid.
//
