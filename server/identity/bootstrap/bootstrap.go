...
// RegisterPublicRoutes wires DELETE /api/session (logout) plus the pre-auth OIDC + break-glass routes (when configured). There is no
// POST /api/session; sessions are minted by the OIDC callback or the break-glass FinishLogin / FinishSetup endpoints.
...
// RegisterAuthedRoutes wires GET /api/session (who-am-i), GET /api/audit-events (operator-action history), and the break-glass reauth
// POST endpoints. Caller wraps in SessionMiddleware + CSRFMiddleware before mounting.
