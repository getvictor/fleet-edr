...
// ReauthChallenge tells the UI which reauth flow to run when the chokepoint denies with reason=reauth_required. The UI reads
// AuthMethod to decide between break-glass POST (local_password) and OIDC redirect (oidc); ReauthURL is the absolute path to navigate
// to (oidc) or POST against (local_password).
type ReauthChallenge struct {
	AuthMethod string `json:"auth_method"`
	ReauthURL  string `json:"reauth_url"`
}

// ReauthChallengeFor builds the challenge payload for the actor on ctx. OIDC actors get the bare /api/auth/login?reauth=1 URL;
// the UI is responsible for appending its own &next=<original-path> so the post-reauth redirect lands the operator back on the page
// that triggered the reauth_required deny. Break-glass actors get the POST endpoint URL the UI submits credentials against. When no
// actor is on ctx (the no-actor reason path), a zero challenge is returned.
...
// writeReauthRequired emits the 403 + body shape the UI's useReauthRetry wrapper detects: error="reauth_required" plus an embedded
// challenge object that pins the per-flow reauth URL.
