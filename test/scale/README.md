...
If any criterion fails, the driver exits 2 and the JSON report's `pass` field is `false`. Per-host breakdowns (last error, per-host observation count, per-host p99) are preserved under `per_host` so a single bad node does not look identical to a systemic regression.
...
Set `--pass-max-queue-depth=N` to gate on max queue depth (any host crossing N flips `pass` to false). The default 0 leaves the gate disabled until an operator has captured baseline values worth gating on.
