...
test.describe.serial("reauth modal retry after stale-session denial", () => {
  ...
  test("OIDC reauth: modal → dex re-auth → operator re-click → resolved", async ({
    ...
    try {
      ...
      // rows out of the in-memory list on a successful update (see
      // ui/src/components/AlertList.tsx). Row vanishing is the
      // operator-visible success signal; we cross-check the actual
      // state in the DB + audit assertions below.
      await expect(
        page.getByText("OIDC reauth modal — critical alert"),
  ...
  test("break-glass reauth: modal → password+WebAuthn → auto-retry → resolved", async ({
    ...
    try {
      ...
      // AlertList's applyStatus then filters the now-resolved row
      // out of the in-memory list (statusFilter="open" by default).
      // Row disappearance is the operator-visible success signal;
      // the DB + audit blocks below verify the underlying state.
