...
That exactness covers the COUNTERS, and only up to the at-least-once retry described below. The times a statistics row carries are taken when it is written, so deferring the write attributes work up to one flush interval later than it happened, and work in the final seconds of a day can be recorded against the next one. This is a bounded and accepted difference, not an exactness claim: the interval is seconds against a window read in days, so no decision the numbers exist for turns on it. A flush that FAILS widens that shift to the length of the outage rather than one interval, because the times come from the eventual successful write, and a long enough outage can therefore attribute work outside the window a reader is looking at.

A failed flush SHALL retain its statistics and retry them on a later flush, so a transient database failure DISCARDS no counts. Retrying an additive write is at-least-once rather than exactly-once: a commit whose result never reaches the client is added again, inflating that flush's contribution. The inflation is bounded to one flush and leaves the derived mean unmoved, because the count and the total inflate together, which is the same property a replayed batch already relies on. Under-counting is the direction that misleads a reader, so the system SHALL prefer the bounded over-count to dropping. Because the pending set is keyed by rule, retaining it SHALL NOT grow with the number of failed attempts. Statistics are lost only in conditions that SHALL be documented where an operator reading the table would look, and there are three rather than the one an earlier draft claimed: an ungraceful shutdown; a graceful shutdown whose final write does not complete within its own short budget, which a database outage spanning the shutdown produces; and work recorded after that final write, since the evaluation workers are not themselves joined to it. The last is bounded by whatever a single in-flight batch had produced.
