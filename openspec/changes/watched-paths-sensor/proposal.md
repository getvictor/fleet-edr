...
- **No firehose, even from a bad push.** A prefix at the top of the filesystem (`/`, `/Users/`, or `/etc/` through `/private`) is skipped by the extension itself, not only refused by the server.
...
- A pushed path that fails to mute is logged, and that update then unmutes nothing, so a failure never leaves a previously watched path unwatched; the next update tries the path again. A built-in path that fails stays fatal, as it was, because after inversion it would silently go unobserved.
