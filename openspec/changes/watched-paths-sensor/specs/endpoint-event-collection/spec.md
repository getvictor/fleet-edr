...
The extension SHALL apply a set only when its `version` or its `epoch` is ahead of the last set it accepted, and SHALL otherwise leave the active and persisted sets unchanged. Commands can reach the host out of order, so without this an older set delivered late would replace a newer one; `epoch` keeps ordering sets after a server database restore sends `version` backwards. This is the rule application control uses for its policy snapshots, and the two SHALL keep one rule: ordering by `epoch` first would order sets by the server database's wall-clock time rather than by its own counter. A command queued before a restore cannot outrank the restored set, because the restore brings back the pending commands with the set, so none carries a `version` above the restored one, and the next replacement takes that `version` plus one with a later `epoch`.
