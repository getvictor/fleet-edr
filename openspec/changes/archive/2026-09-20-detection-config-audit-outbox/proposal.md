...
- **The watched-path row keeps its push counts.** A replacement's audit row reports how many hosts the set was queued for, which is known only after the transaction commits. Its entry is written held and released by the writer once it adds the counts. If the writer stops first, the hold lapses after five minutes and the entry is delivered without the counts rather than not at all. A seal that arrives after the hold has passed is refused rather than racing a drain that may already have read the entry.
