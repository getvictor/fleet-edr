...
- **Who changed it, by name.** The GET and PUT responses carry `updated_by_label`, the display label resolved from `updated_by` the way the watched-path set already resolves its author, so the section names a person or service account. It falls back to the principal id when the account is gone. Without it this page would be the only place in the product that answers "who widened what every contained host can reach" with `usr_1`.
