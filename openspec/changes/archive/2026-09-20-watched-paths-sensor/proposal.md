...
- **Newest set wins.** Commands can reach a host out of order (a poll returns pending commands newest first, and the poll and the control stream can overlap), so the extension applies a set only when it is ahead of the last one it accepted, ordered by its epoch (the server's update time, forced past the previous one) and then its version. Epoch is what still orders sets after a server database restore sends versions backwards, and application control orders `policy_epoch` the same way.
