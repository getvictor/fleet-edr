...
- [x] 10.5 Manual QA on the dev server (Chrome MCP + real HTTP API against the running `task dev:server` on https://localhost:8088, real MySQL + worker/tester): verified in a real browser that the gated Webhooks settings page renders, a destination is created through the form ("Saved.", write-only password-masked secret, row reflects the live API), and "Send test" surfaces the delivery outcome inline. Verified over the API: CRUD (create/list/update/delete), secret never echoed (`secret_set` only), the four SSRF save-rejects (http, loopback literal, cloud-metadata, RFC1918 -> 400), test-send transport timeout, and `webhook.manage` RBAC (analyst -> 403 on read + create). The original "point at a local receiver + fire an alert" wording is infeasible by design: the SSRF guard blocks all loopback/private targets, so the authoritative delivery-time refusal was verified instead (test-send to `https://localhost` -> `DialControl` "refusing to connect to blocked address ::1"). The successful signed-receipt round-trip (receiver recomputes HMAC over `id.timestamp.body`) and the alert-fired enqueue + delivery-status readout are covered by the automated tests (`webhook/client_test.go`, `tests/webhook_worker_test.go`, `tests/webhook_enqueue_test.go`) since both need a receiver on a non-blocked address / the full event pipeline.
