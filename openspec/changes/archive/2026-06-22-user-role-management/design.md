...
1. **Last-admin protection.** Demoting or disabling the last active user holding `admin` or `super_admin` is rejected (`409 last_admin`). Prevents locking the deployment out of its own user management. The check and the mutation run in one transaction that first takes an exclusive row lock on the admin-tier rows of the `roles` table (the serialization sentinel) under READ COMMITTED, then re-counts the other active admins, then mutates. This is race-free: every admin-count-reducing mutation contends on the same sentinel rows, so concurrent demotions/disables run one at a time and each re-counts against the prior one's committed effect (see the trade-offs note for why locking the other admins' binding rows would not work). Lives in the `rbac` store (`SetUserRole` / `SetUserStatus`), which owns the admin-count invariant.
...
- Last-admin protection serializes on an exclusive row lock over the admin-tier `roles` rows rather than a DB constraint. The alternative considered (and rejected) was locking the *other* admins' binding rows with `SELECT ... FOR UPDATE`: that does not serialize, because two demotions of different targets exclude different rows and lock disjoint sets, so they never contend. The sentinel-row lock is the standard "serialize a global invariant" pattern; READ COMMITTED is required alongside it so the waiter's re-count sees the holder's committed effect rather than a stale REPEATABLE READ snapshot. The cost is that admin-role binding writes briefly serialize (and a concurrent admin-role insert can wait on the FK shared lock), which is negligible for this low-write surface.
