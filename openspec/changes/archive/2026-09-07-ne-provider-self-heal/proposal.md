...
- **The host app's signing identity is verified before the agent runs it as root.** Running that path as root is new here, and it is reachable by a non-root attacker: the bundle and its binary are root-owned, but `/Applications` itself is `drwxrwxr-x root:admin` on a stock install, and renaming a directory needs write on the parent, so any admin user can move the real bundle aside and substitute their own. The agent therefore requires the host app to carry the same team identifier it carries itself before executing it, which turns that from an admin-to-root escalation into a refusal.
