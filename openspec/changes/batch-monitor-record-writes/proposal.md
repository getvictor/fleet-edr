...
- **The store writes them together.** `InsertMonitorRecords` reads the archive once for the union of the records' triggering events. It writes rows, event links and evidence copies with chunked multi-row inserts, 100 records per transaction. Each record gets exactly what the alert insert wrote for it: the same row, dedup key, links and evidence, and no webhook delivery. A failed write fails the batch; the retry's write is a no-op for anything already committed, because of the dedup key.
