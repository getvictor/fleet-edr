...
`evalExecArm2` calls `findShellOnExecChain`, the walk the connect arm already used. The temp arm therefore takes the newest suitable generation and defers on incomplete ancestry.
...
The first change can only add detections. The second removes one class: a chain whose shell's parent is genuinely missing from the graph. That is the trade the connect arm already made, and it is made here for the same reason.
