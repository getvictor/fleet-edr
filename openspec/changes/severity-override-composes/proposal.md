...
The banding is Elastic's, which is the one most operators reading this product already have a feel for: low 0-21, medium 22-47, high 48-73, critical 74-100. Each band is represented by a point inside it rather than its edge, so a delta lands within a band instead of teetering on a boundary. Those points are 15, 35, 60 and 85. They are neither the band midpoints nor evenly spaced; what they are chosen for is that adding 25 crosses exactly one band boundary from each of the three non-critical ones, so `dns_c2_beacon`'s DGA escalation of 25 advances a finding by one band wherever it starts, and reaches critical from high, which is what the untuned rule has always reported.
