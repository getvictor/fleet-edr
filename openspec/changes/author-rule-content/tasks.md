...
- [x] A write that fails BEFORE it is applied leaves both the document and the version untouched, pinned by a test that forces the failure with a path longer than the column holds rather than by inspection. The claim stops there deliberately: a commit can succeed with its response lost, so an error does not prove the write did not apply, and the spec says atomicity is a property of the corpus rather than a promise about what an error means.
- [x] Two writers holding one version serialise, so exactly one wins. Covers the meta-row lock, which the sequential tests could not: removing `FOR UPDATE` now fails with two documents stored where one was allowed.
- [x] A path that is not canonical is refused. `FS` strips one leading slash while storage keys the raw path, so `/x.yml` and `x.yml` persist as two rows and collapse to one document at load.
...
- [x] Rule identities are compared case-insensitively, matching the collation of the columns that persist them. Fixed in the loader rather than only in the validator, because rule identity is the loader's invariant and it holds for every load path. The vendored corpus was checked for existing collisions first: none among its 69 rules.
