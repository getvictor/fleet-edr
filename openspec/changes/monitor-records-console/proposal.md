...
- **The records page explains the numbers.** Records collapse repeat matches on the same process and expire after a limited time, so there can be fewer records than the count. Stated where the two meet, so the difference does not read as lost data. The Observed note says the same.
