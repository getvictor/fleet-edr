...
- Affected specs: `web-ui` (MODIFIED: The host detail surfaces the health conditions; ADDED: The process detail omits a self-referential alert link).
- Affected code: `ui/src/components/HostHeader.tsx` (+ `.scss`), `ui/src/components/ProcessTree.tsx` (+ `.scss`), `ui/src/components/ProcessDetail.tsx`; removed `ui/src/components/HostHealthPanel.{tsx,scss,test.tsx}`; comment scrub in `ui/src/components/ActivityHistogram.tsx`.
