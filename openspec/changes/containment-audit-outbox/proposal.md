...
- **An outbox in the response schema.** Migration 00006 adds `response_audit_outbox`, one table for the whole context so its audit rows keep a single order. A containment change writes its audit entry there in the transaction that records the state and queues its command (#1073), so the entry commits if and only if the change does. A drain turns entries into audit rows: the request asks for its entry right after the change, and the sweep delivers it, along with whatever no request asked about, every minute. Delivery is at least once and never drops an entry.
