...
- [x] 1.11 Sweep the other active SPEC deltas for the same framing, which the boundary below puts in scope. One needed it: the per-rule evaluation-statistics requirement contrasted itself with "recording only on acknowledgement" and said a retryable outcome could never be recorded, which a terminal withdrawal now makes not-quite-true.
- [x] 1.10 Scope the sweep to code and to SPEC deltas, and leave the proposal and task documents of other changes alone. Those describe what a change proposed and did at the time, `openspec archive` moves them into the archive unchanged, and rewriting them to describe behaviour they did not introduce would falsify that record rather than correct it. The requirement is the surface that has to be coherent, and it is.
