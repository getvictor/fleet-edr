...
Exactly-once needs no coordination between workers, and it comes from set-aside being TERMINAL rather than from anything about one transaction's own rows. The withdrawing statement matches rows in the pending state, and a withdrawn row is not pending and is never returned to pending, since the statement that resets a row requires it to be in flight. So a row makes that transition once in its life, row locks serialise concurrent attempts at it, and only the caller whose statement performed it is told the row was withdrawn.
