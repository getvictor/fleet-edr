...
- **GIVEN** the host page entered for an alert whose alerted process is absent from a process tree that loaded successfully and contains other processes
- **WHEN** the operator switches to the timeline view
- **THEN** the timeline lists the full host event stream
- **AND** it states that the alerted process is not in the time window
- **AND** it does not attribute the fallback to missing generation data
