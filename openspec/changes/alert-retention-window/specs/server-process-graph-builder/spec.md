...
The change from the prior requirement is that the alert reference which keeps a record is no longer permanent: an alert now expires, and the record it held becomes eligible when it does.
