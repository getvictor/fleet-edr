...
- **Observability parity with MySQL (ADR-0006, OTel only).** The ClickHouse `database/sql` connection is opened through the same `otelsql` wrapper as MySQL (`server/bootstrap/db.go`), tagged `db.system=clickhouse`, with `otelsql.RegisterDBStatsMetrics`. Every archive query and insert then gets a span plus the connection-pool / `db.sql.*` RED metrics with no bespoke code, identical to the MySQL path. Archive-specific signals (`async_insert` flush-ack latency, batch row counts) are layered on top. If a later perf pass moves inserts to clickhouse-go's native protocol instead of `database/sql`, instrumentation switches to clickhouse-go's built-in OTel hooks for the same signals.
