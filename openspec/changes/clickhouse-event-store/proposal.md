...
- **Affected code:** new `server/visibility/` context (ingestion + `internal/clickhouse` event archive + `internal/eventlog` MySQL queue + `api/` with the `EventLog`/`EventArchive` published interfaces); `server/detection` becomes a consumer of `visibility/api`: its pipeline claims from the queue, and its cross-stream correlation (`GetNetworkEventsForProcess`) now fetches events from the archive via `visibility/api` because the events it used to read from its own MySQL table now live there. The process-graph reads and the operator visibility reads stay in `detection`. New MySQL migrations (`event_queue`, `alert_event_payloads`, drop `events`); new embedded ClickHouse migrations; `server/migrations/runner` gains the goose ClickHouse dialect; `docker-compose.yml` gains a ClickHouse service; `go.mod` promotes `clickhouse-go/v2` to a direct dependency.
