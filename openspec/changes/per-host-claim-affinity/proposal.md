...
- Worker concurrency is clamped to half the connection pool. A worker inside the critical section holds two pooled connections at once, the one `GET_LOCK` pins plus the one the claim and flush use, so a pool smaller than twice the worker count lets every worker hold a lock connection while waiting for a claim connection that no one will release. That failure mode is a silent stall with nothing logged, so the clamp trades throughput for liveness and logs the reduction.
- With no coordinator wired the processor runs a single worker, which upholds the same ordering guarantee without a lock.
