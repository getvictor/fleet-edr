...
- [x] The signature read is captured before extraction rather than piped into `sed`. This script is `/bin/sh` with `set -eu` and no portable `pipefail`, so a piped read would let a codesign failure be masked by `sed` succeeding: the identifier comes back empty and the mismatch branch blames the identifier when the truth is that codesign could not read the binary, with codesign's diagnostics filtered into the pipe and lost. The two failures now have distinct exits (16 read failure, 15 mismatch).
- [x] Checked the assertion actually catches the regression rather than only passing: signing the same binary WITHOUT `--identifier` yields `agent-wrongid-55554944199156cd6ff53f65ffb04bacdbc0a936`, which the comparison rejects with exit 15. Pointing the same check at a missing binary exits 16 and prints codesign's own `No such file or directory`, so a read failure is not misreported as a wrong identifier. A guard that has only ever been observed passing is not evidence it can fail.
