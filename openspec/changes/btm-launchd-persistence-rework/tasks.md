...
- [ ] (Follow-up) Populate `is_notarized` via a `SecAssessment` Gatekeeper check.
...
- [x] `privilege_launchd_plist_write` decides on `executable_code_signing` (platform / notarized / managed / allowlist),
      not the instigator; emits a process-less finding with a `launchdaemon:<item>` subject.
...
- [ ] edr-qa L5 on a notarized RC: confirm a notarized vendor daemon is trusted and the unsigned dropper fires
      (ESF signing is redacted on edr-dev, #187).
