...
- [x] Document all THREE `Rule.Evaluate` error classes on the public interface: ordinary (discard, isolate), not-yet-materialized (keep, finish the batch), read-unavailable (keep, stop the batch). The contract previously described two and told authors to finish the batch on the retryable one, which is wrong for the third.
- [x] State that preservation covers alert-raising findings, not monitor-mode counts, wherever it is described. Monitor counts for a set-aside batch are issue #843.
