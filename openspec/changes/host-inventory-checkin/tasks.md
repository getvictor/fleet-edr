...
- [x] 1.1 Add `Inventory` struct (`hostname`, `os_name`, `os_version`, `os_build`, `agent_version`) as an optional pointer field on `StatusReport` in `server/endpoint/api/status.go`; PBT round-trip test (`Marshal ∘ Unmarshal == identity`)
...
- [x] 2.2 Include inventory in the poster's payload (collected once at startup; OS/hostname changes are picked up on agent restart, which is when they occur in practice: OS upgrade reboots, agent upgrade restarts; re-collect cheaply per post only if trivial)
