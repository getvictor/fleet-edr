...
- [x] `ReportSeverity`: how prominent a line is in the system log, kept as a SEPARATE axis from the stream. A staged upgrade's reboot notice (#399) is normal output an operator reads on stdout, but it is also the one line worth finding again when filtering the log by level, so routing it through the same `progress` call as every routine line would have quietly downgraded it from `warning` to `info`. Review caught exactly that; the requirement calls it a DISTINCT operator-facing signal, and identical severity is not distinct.
...
- [x] The severity axis: a routine progress line and the reboot notice logged back to back must come out `info` then `warning`. Mutation-checked too, since this test exists because the regression it pins had already been written once.
...
- [x] `xcodebuild` Debug build green; `swift test` 229 tests; `swiftlint --strict` 0 violations; `spectrace check --strict` 775/775; `openspec validate --strict`.
