...
#### Scenario: The status says whether names are filtered

- **GIVEN** a contained host
- **WHEN** the extension reports its status
- **THEN** the status says whether its DNS proxy was running, and so whether the restriction on which names resolve was in force
