...
While the host is contained, the network extension's DNS proxy SHALL forward a DNS query only when it is a single-question query for one of the lifeline's names, compared case-insensitively and without a trailing dot label by label, and SHALL forward it rebuilt from its ID, opcode, recursion-desired flag and question, with every other flag clear and no records except one the proxy builds itself, so nothing a process appends after the question leaves the host; the query's ID and destination remain the process's choice. When the client offered an OPT record the rebuilt query SHALL carry one, holding a UDP payload size and nothing else. That size SHALL be no larger than the size the client asked for, since the answer returns to that client, and no larger than a size that traverses the internet without fragmentation. A size below 512 SHALL be raised to 512, which is both what a responder treats a smaller advertised size as and a size every stub can receive, so a nonsense size is corrected rather than carried. Without it an allowed answer over 512 octets returns truncated, the stub retries over TCP, and TCP is closed while contained, so a name with many addresses stops resolving. An OPT record carrying an option, an EDNS version the proxy does not know, an extended response code, the DNSSEC-OK bit or any other flag, and any record that is not a lone minimal OPT, SHALL leave the query rebuilt without one rather than forwarded unread. It SHALL answer every other query locally with REFUSED, carrying the query's ID and question and no records. A datagram that is not a single well-formed query SHALL be neither forwarded nor answered, and DNS over TCP SHALL not be resolved: a session is refused when it starts, and one opened before the host was contained is closed on its next query. A containment whose lifeline names no host name resolves nothing. The filter's lifeline allows DNS so the agent can resolve the server, to the host's configured resolvers alone, and every lookup on a host whose DNS proxy is running passes through this proxy. The two restrictions are separate and only the filter's holds unconditionally: the proxy restricts which names resolve, and a host whose DNS proxy is disabled or stopped resolves any name its configured resolvers answer. A host that is not contained SHALL have its DNS forwarded as before.
...
- **THEN** the proxy forwards the query with an OPT record of its own holding a size no larger than the one offered and no larger than a size that traverses the internet unfragmented, and no smaller than 512 when the one offered was smaller still
- **AND** an OPT record carrying an option, an unknown version, the DNSSEC-OK bit or any other flag leaves the forwarded query without one
