...
Attempts are reported rather than batches, matching how they are recorded: a replayed batch really did evaluate again. The derived mean is unaffected, since the time inflates by the same factor, but the column says what it is counting so the number is not read as a fire count.
