...
- **AND** the tunnel is established in the protocol that address names, whichever proxy scheme the lifeline target was derived for
- **AND** a proxy addressed over TLS is spoken to over TLS before anything is written, so its credentials never cross the path in the clear
