...
The worst case per hold interval falls from every arriving query stalled for 3s to at most the probe budget, and the interval itself grows while the upstream stays wedged.
...
- Probing never stalls more than the probe budget of client queries per hold interval.
