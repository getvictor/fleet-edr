...
Hovering a process node in the tree SHALL show the process's full command line and its derived code-signing verdict without opening the detail panel. The verdict SHALL be derived from the code-signing fields the node carries and SHALL distinguish: unsigned (no code-signing block reported), ad-hoc (the ad-hoc signing flag set), Developer ID with the team identifier (a non-empty team id), Apple platform (the platform-binary flag), and signed (a signing identity with none of the above). Nodes whose verdict is ad-hoc or unsigned SHALL carry a visible marker in the graph, distinct from the alert styling. For an aggregated node, the hover SHALL state the group size and show the representative process's command line and verdict.
