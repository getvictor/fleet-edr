...
This capability defines the EDR server audit trail. Every authentication outcome and every authorization decision on a
state-changing action writes an audit row, with read-only actions sampled at a configurable fraction (and always recorded
for break-glass actors). Each audit row is dual-emitted as a structured slog record and OTel span attributes so the
observability pipeline sees it even when the database insert fails. The audit table is append-only at the application
layer: no production code path updates or deletes a row, so a future dual-database-user setup can revoke write privileges
without code changes. The audit log is itself readable through an authorized endpoint that audits its own access.
