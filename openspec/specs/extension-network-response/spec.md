...
Defines the network extension's enforcement behavior: a default-forward posture that denies only on explicit policy match, local answering of blocked DNS queries, a health watchdog with policy-aware bypass that fails open for resolution when no blocking policy is active while still enforcing active blocks, and declarative host network containment that survives a provider restart.
