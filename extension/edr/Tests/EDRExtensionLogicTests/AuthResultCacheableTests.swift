...
final class AuthResultCacheableTests: XCTestCase {
    ...
    func testFullyResolvedAllowIsCacheable() {
        ...
        XCTAssertTrue(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .computed("abc"), leafCertResolved: false, snapshotHasCertificateRules: false))
        // No BINARY rules (hash not needed) + certificate rules present but the leaf cert was resolved.
        XCTAssertTrue(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .notNeeded, leafCertResolved: true, snapshotHasCertificateRules: true))
    }
    ...
    func testColdMissAllowIsNotCacheable() {
        ...
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .deadlineExceeded, leafCertResolved: true, snapshotHasCertificateRules: false),
            "a deadline-exceeded allow must not be cached")
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .readFailed, leafCertResolved: true, snapshotHasCertificateRules: false),
        ...
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .computed("abc"), leafCertResolved: false, snapshotHasCertificateRules: true),
    ...
    func testUndecidedAllowIsNotCacheable() {
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.allowWithUndecidedAudit(reason: .deadline)),
    ...
    // spec:extension-application-control/decided-allow-is-cached-and-flushed-on-snapshot-replacement/a-detect-match-allow-is-not-cached
    func testAllowThatMatchedADetectRuleIsNotCacheable() {
        let rule = makeRule(
            ruleType: ApplicationControlRuleType.teamID, identifier: "EQHXZ8M8AV", enforcement: ApplicationControlEnforcement.detect
        )
        let reported = AuthEvaluation(decision: .allow, wouldBlock: RuleMatch(rule: rule, matchedIdentifier: "EQHXZ8M8AV"))
        XCTAssertFalse(authResultIsCacheable(
            reported, hashOutcome: .notNeeded, leafCertResolved: true, snapshotHasCertificateRules: false),
            "a cached allow never reaches the handler again, so later execs of the binary would go unreported")
        XCTAssertTrue(authResultIsCacheable(
            evaluation(.allow), hashOutcome: .notNeeded, leafCertResolved: true, snapshotHasCertificateRules: false),
            "the same fully resolved allow without a DETECT match is cached")
    }
    ...
    func testDenyIsNotCacheable() {
        ...
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.deny(rule: rule, matchedIdentifier: "c0")),
        ...
        XCTAssertFalse(authResultIsCacheable(
            evaluation(.denyWithUndecidedAudit(reason: .deadline)),
            hashOutcome: .deadlineExceeded, leafCertResolved: true, snapshotHasCertificateRules: false),
