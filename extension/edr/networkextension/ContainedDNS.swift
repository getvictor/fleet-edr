...
enum ContainedDNS {
    ...
    /// a single-question query whose name is one of the lifeline's names, compared case-insensitively and without a trailing dot, and
    /// answers every other query REFUSED. A datagram that is not a well-formed query is dropped.
    ...
    /// datagram or uses a compression pointer, none of which a stub resolver sends. The name is read label by label rather than as
    /// text, so a label carrying a dot byte cannot spell the server's name.
    private static func singleQuestion(in data: Data) -> (end: Int, name: String?)? {
        ...
        while offset < bytes.count {
            ...
            guard bytes[offset] < Wire.compressionPointerMarker, offset + 1 + length <= bytes.count else { return nil }
