...
// that anything is actually capturing, which is the bug behind issue #649. Network containment status is level-triggered
// in the same way, so it is re-broadcast on hello too.
XPCServer.shared.start(onPeerConnected: {
    ProviderStatus.shared.publish()
    NetworkContainmentController.shared.publish()
})
