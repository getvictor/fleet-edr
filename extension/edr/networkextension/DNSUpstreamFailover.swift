...
enum DNSUpstreamFailover {
    /// Chooses the next resolver to try after `failedServer` did not answer.
    ///
    ...
    static func nextServer(afterFailing failedServer: String?, systemServers: [String]) -> String? {
        ...
        guard systemServers.contains(failedServer) else { return nil }
        return systemServers.first { $0 != failedServer }
    }
    ...
    static func address(of endpoint: NWEndpoint) -> String? {
        ...
        return rendered.split(separator: "%", maxSplits: 1).first.map(String.init)
    }
