...
/// AtomicFile is the one way the extensions persist a pushed document: the application-control snapshot, the watched-path set, and the
/// network containment state. Shared so the stores cannot drift apart on how a write survives a crash or on what a missing directory
/// means.
enum AtomicFile {
