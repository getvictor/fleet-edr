...
- The project builds on Go 1.27+ (see `go.mod`); modern language and standard-library features through 1.27 are in-bounds. Integer range (`for i := range N` where `N` is an `int`) compiles and is the preferred form, and newer stdlib such as `strings.SplitSeq` / `bytes.SplitSeq` is available. Do NOT claim these "don't compile" or "break Go 1.22+": that has been a recurring false positive (#239, #344).
