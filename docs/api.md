...
The scheme is always `https://`: the server has no plaintext-HTTP mode
(issue #140) and dev deployments use a self-signed cert generated by
`task dev:certs`. Examples in this doc use `https://`.
