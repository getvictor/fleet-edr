...
Scope is the system as it ships: Go agent + Swift system extension + Swift network extension on macOS endpoints; a Go server that can run as one combined `fleet-edr-server` binary or split into a standalone `fleet-edr-ingest` tier; a MySQL control plane plus a ClickHouse event archive (ADR-0015); an embedded React UI; a persistent agent control channel (a bidirectional gRPC stream multiplexed onto the same HTTPS listener, ADR-0016); operator and non-human service-account API auth (ADR-0013) resolving to one unified principal model (ADR-0017); and MDM-driven deployment. Out of scope is everything in the closing section.
...
- **The MySQL instance is on a private network.** MySQL holds the control plane, the derived process graph, the ephemeral `event_queue` work queue (ADR-0016), alerts and their self-contained evidence copies, sessions, and principals; the raw event firehose is in ClickHouse (ADR-0015). Direct DB-network compromise is out of scope; anything past the Compose network is the operator's responsibility.
...
| Information disclosure | DB compromise exposes credentials / retained data. | Argon2id for passwords; self-validating signed host tokens keyed off `EDR_SECRET_KEY` and SHA-256-hashed service-account credentials, neither verifiable nor forgeable from a DB-only leak. MySQL does not hold the raw event firehose (ADR-0015); it retains the process graph, alerts, and the per-alert evidence copies in `alert_event_payloads` (which do contain event payloads). **GAP, medium**: no application-level encryption at rest for those payload copies; relies on storage-layer encryption (InnoDB tablespace encryption, host filesystem encryption), a deployment-time choice. The same at-rest concern for the full firehose applies to ClickHouse (component 7). |
| Denial of service | Disk fills with retained rows. | The retention runner (`server/detection/internal/pipeline/retention.go`) prunes MySQL process records (`EDR_RETENTION_DAYS`, default 30), skipping rows an alert still references, alerts with their event links and evidence copies (`EDR_ALERT_RETENTION_DAYS`, default 180 days after last triage activity), and monitor records, which outnumber alerts several times over (`EDR_MONITOR_RETENTION_DAYS`, default 7). An expiring alert or monitor record releases the process record it held. Each prune runs in bounded batches, counted by `edr.retention.processes.rows_deleted`, `edr.retention.alerts.rows_deleted`, and `edr.retention.monitor_records.rows_deleted`. **GAP, low**: a window of 0 disables that prune, and then those rows grow without bound; the monitor window is the one where that happens fastest. Raw-event disk growth is bounded by ClickHouse native TTL (component 7); the ephemeral `event_queue` is pruned by a leader-gated sweep after ack (ADR-0016). |
...
The persistent control channel (ADR-0016) is a bidirectional gRPC stream that pushes commands to a connected host in real time and carries outcomes back. It is the primary transport; the `GET /api/commands` short-poll remains as the degraded floor. Its server side (`server/response/internal/gateway`) is multiplexed onto the same HTTPS listener and port as REST/UI: one native HTTP/2 server dispatches `application/grpc` requests to the gateway and everything else to the REST/UI handler, so there is no separate control address to configure or firewall.
...
Last verified by a full code-driven boundary inventory on 2026-07-05. Review history is in the git log for this file.
