...
| Denial of service | Disk fills with retained rows. | The retention runner (`server/detection/internal/pipeline/retention.go`) prunes MySQL process records (`EDR_RETENTION_DAYS`, default 30), skipping rows an alert still references, and alerts with their event links and evidence copies (`EDR_ALERT_RETENTION_DAYS`, default 180 days after last triage activity). An alert's expiry releases the process record it held. Both prunes run in bounded batches, counted by `edr.retention.processes.rows_deleted` and `edr.retention.alerts.rows_deleted`. **GAP, low**: a window of 0 disables either prune, and then that table grows without bound. Raw-event disk growth moved to ClickHouse native TTL (component 7); the ephemeral `event_queue` is pruned by a leader-gated sweep after ack (ADR-0016). |
