...
| Elevation of privilege | A network attacker uses the root-running agent to execute arbitrary code. | Agent only executes typed commands (`kill_process`, `set_application_control`) fetched from the server, authenticated by the per-host bearer token. The command-type → handler mapping is exhaustive (`agent/commander/commander.go`); unknown types are rejected. `set_application_control` carries an operator-authored Application Control blocklist the agent applies but cannot itself originate: it is gated by the RBAC chokepoint, versioned, and audited server-side (see the Detection-content row below). |
...
| Compromise of the OIDC client secret or the deployment root secret. | Both are loaded via `*_FILE` paths (Docker-secrets convention) so they are not in env-listing output. Rotating the OIDC client secret in the Single sign-on settings UI takes effect without a restart (the `EDR_OIDC_*` env vars are seed-only once SSO is configured); sessions stay valid because they are signed with a key derived from `EDR_SECRET_KEY` (a separate 32+ byte secret), not the client secret. Rotating `EDR_SECRET_KEY` invalidates every existing session (operators sign back in via OIDC) and every host token (agents re-enroll automatically), since both the cookie signing key and the host-token pepper derive from it. |
