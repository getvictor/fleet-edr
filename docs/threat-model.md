...
| Denial of service | An ingest spike or unbounded growth overruns the archive. | Writes are batched via the native batch protocol (one prepared INSERT per block), never a synchronous per-request write; the MySQL `event_queue` decouples ingest from detection (ADR-0016) so an ingest burst drains rather than stalls detection or loses data; native TTL plus a designed-in hot-to-S3 tiered-storage policy bound disk. Cold-partition read spikes on UI-synchronous correlation reads are a named watch item (ADR-0015), gated by the #203 load test. |
