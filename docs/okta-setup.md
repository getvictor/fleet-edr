...
| `error=unknown_subject` with JIT enabled | Okta issued a `sub` for a user that the EDR is treating as new (usually a duplicate-user race); check `users` for an earlier row with the same `email` | Delete the orphan row (JIT is always on; the settings page has no toggle, so this is the only fix in the current release) |
