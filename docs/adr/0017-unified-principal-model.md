...
- **One string identifier, everywhere.** A principal id is a type-prefixed opaque string (`usr_...`, `svc_...`, and the singleton `sys`). It is the `principals` primary key, the value every attribution column stores, the actor id on every audit row, and the id on `api.Actor`. The `type` is both encoded in the prefix (self-describing on the wire and in the database) and stored as a column (the authoritative, indexable discriminator). The local part is opaque: stable, unique, and never reused. Within the identity context, attribution columns are real foreign keys to `principals(id)`; across context boundaries the same string is stored without a foreign key (ADR-0004), resolved for display through a single principal resolver.
