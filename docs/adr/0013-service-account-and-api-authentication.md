...
- **Least privilege + audit.** A service account is bound to a role (never `super_admin`); the token is audience-bound; issuance and use are audited. Managing service accounts is gated behind new `service_account.*` actions held by `admin`/`super_admin`.
