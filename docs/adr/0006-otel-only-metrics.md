...
- [`operations.md`](../operations.md) documents the metric names operators monitor (`edr.events.ingested`, `edr.alerts.created`, `edr.enrolled.hosts`, `edr.offline.hosts`, `edr.retention.rows_deleted`, `edr.processes.ttl_reconciled`, `edr.agent.queue.dropped`). These follow OTel semconv spelling, not Prometheus snake_case conventions. DB-client latency + connection metrics come from the standard otelsql driver instrumentation (`db.sql.latency`, `db.client.connection.*`), not a bespoke metric.
