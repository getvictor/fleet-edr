...
- **`NOTIFY_RENAME` was deliberately NOT subscribed.** The ADR listed it parenthetically, but visudo/sudoedit write via temp-file + atomic rename onto `/etc/sudoers`, so watching rename would fire on every legitimate sudoers edit. The atomic-replace gap stays documented on `sudoers_tamper` (unchanged from before). Closing it cleanly (with a visudo/sudoedit/installer carve-out) is a possible focused follow-up. Step 4 (server-pushed watched-path set) remains future work; see the 2026-09-13 amendment.

## Amendment 2026-09-13: the watched set takes paths pushed by the server (step 4, sensor half)

Issue #998 starts step 4. The file-tamper client's watched set is now the built-in sudoers paths plus a set the server pushes over a `set_watched_paths` command and a `watched_paths.update` XPC message. The server half (storing the set, bounding it, pushing it to hosts) and the console half follow in their own changes.

- **The built-in paths cannot be removed by configuration.** A pushed set only adds to them, because the shipped sudoers rules depend on them. An empty pushed set is exactly the behaviour before this change.
- **A set is applied to the running client by difference.** `es_mute_path` for what the new set adds, then `es_unmute_path` for what it drops, under the inversion already in place. A path in both sets is never unmuted, so replacing a set opens no gap in coverage. No restart and no re-subscription is involved.
- **The set persists** at `/var/db/com.fleetdm.edr/watched-paths.json` before it is applied, and the client starts from it, so a restarted extension does not fall back to the built-in paths until the agent next pushes. A set that cannot be written is not applied.
- **The newest set wins.** A set carries a version and an epoch (the server's update time), and the extension applies one only when either is ahead of the last it accepted, so a command delivered late cannot put an older set back. The epoch keeps ordering after a server database restore, as `policy_epoch` does for application control (#322).
- **The extension holds the server's entry rules itself.** Every rule the server applies to an entry is applied again on decode (absolute, clean, no control characters or NUL, a prefix below a top-level directory), because a mute reaches the kernel as a C string and a NUL or `..` segment could otherwise mute a top-level prefix. A server bug cannot reintroduce the firehose this ADR removed.
- **Failure handling splits by origin.** A built-in path that fails to mute stays fatal, as before. A pushed path that fails is logged, because exiting would restart the extension into the same persisted failure, and the update that failed unmutes nothing, so no previously watched path is left unwatched.
