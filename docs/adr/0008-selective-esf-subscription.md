...
- **Failure handling splits by origin.** A built-in path that fails to mute stays fatal, as before. A pushed path that fails is logged and left out, because exiting would restart the extension into the same persisted failure.
