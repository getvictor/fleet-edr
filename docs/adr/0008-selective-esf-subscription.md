...
- **The set persists** at `/var/db/com.fleetdm.edr/watched-paths.json` and the client starts from it, so a restarted extension does not fall back to the built-in paths until the agent next pushes.
...
- **The extension refuses a top-level prefix itself.** A prefix for `/`, a top-level directory, or `/etc/` through `/private` is skipped on decode, so a server bug cannot reintroduce the firehose this ADR removed.
