...
- **Bounded size.** A rule file may be up to 64 KiB and a corpus up to 4096 rules. The whole corpus is revalidated on every edit and reparsed by every replica whenever it changes, so these bound work that is otherwise unbounded. The largest rule shipped here is under 4 KiB.
