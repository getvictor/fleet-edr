...
4. Verify the canonical tree is well-formed and fully traced after archiving:
   ...
   - The archive did not silently discard a restatement:
     ...
     **A finding the diff adds is something this archive broke.** Neither check above can see one: `validate --strict` passes on a requirement that lost half its text, and `check --strict` passes as long as whatever survived still has markers. Four shapes appear: a scenario the archived deltas say should be canonical and is not, normative text the last restatement carried that is not there either, text or a scenario that restatement retired and that is still there, and a requirement an archived change retired that is still in the tree.
     ...
     The comparison rather than the raw output is the point. The tree already carries entries `archive-verify` cannot classify: a requirement that was retired and really is gone is recognised and not reported, but a scenario dropped from a requirement that is still in the tree looks the same whether an out-of-order archive discarded it or an author trimmed it deliberately in a release nobody is auditing now. That is why it reports rather than gates, and why a standing line is not actionable while a new one is. The standing set is tracked in #905, and is being worked down by classifying each finding rather than by muting it: a finding whose behaviour survives elsewhere, or which belongs to a capability that was specified and never built, is recorded in `openspec/archive-verify-exceptions.yaml` with the survivor or issue that makes it correct, and reported in a separate excused section. Those entries DO gate: an entry naming a survivor that no longer exists, or one left behind after the finding it excused was repaired, fails the command, because nothing else in the pipeline would catch a shrinking report. Findings themselves still do not gate.
...
3. Refresh the vendored ATT&CK table if MITRE has published a newer release. `task attack:latest-check` fails when the table in `ui/src/components/attack-techniques.generated.ts` is not cut from the newest Enterprise release; regenerate with `task attack:table ATTACK_VERSION=<new>`, then check `navigatorATTACKVersion` in `server/rules/api/navigator.go` still matches the major and run `task docs:attack-layer` if the layer's declared version moved. This is a release step rather than a CI check because it needs the network: two offline tests in `server/rules/bootstrap` already enforce, on every PR, that the table covers every technique a rule maps and that it agrees with the layer on the version. A stale table is not cosmetic. ATT&CK renames and re-tactics techniques between releases (v19 replaced Defense Evasion with Stealth and Defense Impairment), so shipping an old table means the Coverage page groups techniques under tactics the current matrix no longer has.
4. Pass the automated gates locally (they mirror CI; a local run avoids a failed release build):
...
5. Open the release-prep PR and get it reviewed. The archive is where editing `openspec/specs/**` is expected and legitimate, unlike on a feature branch, so this is the diff a reviewer scrutinizes.
6. Merge to `main`. Everything downstream tags off the merged commit.
...
- The detection-efficacy run: the MITRE-aligned attack corpus, asserting each built-in rule fires within its SLA (detection rate gate) and the noise corpus stays clean (false-positive gate).
