...
4. Verify the canonical tree is well-formed and fully traced after archiving:
   ...
   - The archive did not silently discard a restatement:
     ...
     **A finding the diff adds is something this archive broke.** Neither check above can see one: `validate --strict` passes on a requirement that lost half its text, and `check --strict` passes as long as whatever survived still has markers. Four shapes appear: a scenario the archived deltas say should be canonical and is not, normative text the last restatement carried that is not there either, a canonical scenario that restatement retired and that is still there, and a requirement an archived change retired that is still in the tree.
     ...
     The comparison rather than the raw output is the point. The tree already carries entries `archive-verify` cannot classify: a requirement that was retired and really is gone is recognised and not reported, but a scenario dropped from a requirement that is still in the tree looks the same whether an out-of-order archive discarded it or an author trimmed it deliberately in a release nobody is auditing now. That is why it reports rather than gates, and why a standing line is not actionable while a new one is. The standing set is 68 findings, tracked in #905.
