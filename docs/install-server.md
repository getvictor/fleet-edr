...
2. **MySQL is a single point of failure in the reference stack.** v0.5.1 ships a single MySQL; the customer brings a replicated or managed MySQL for a fully HA datastore. A MySQL outage takes the control plane down regardless of how many server replicas are running (endpoint enforcement still continues, per above).
3. **Single region, single MySQL writer.** There is no multi-region or active-active deployment in v0.5.1; geo-distribution and read-routing are deferred to a later release.
...
| `EDR_ALERT_RETENTION_DAYS` | no | 180 | How long an alert is kept after its last triage activity, 0 disables. Independent of `EDR_RETENTION_DAYS` in both directions. See [alert retention](operations.md#alert-retention) |
