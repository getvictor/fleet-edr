...
A rule's `enforcement` is `PROTECT` or `DETECT`. A `DETECT` rule does not block: when an exec matches it, the exec runs and the host reports the match, which the server keeps as a monitor record under the rule's id (`app_control:<id>`) with the rule's severity. Use it to see what a rule would block before it blocks anything, then promote the rule to `PROTECT`.
...
Set `enforcement` when you create a rule (`POST /api/v1/app-control/policies/{id}/rules`); a request without it creates a `PROTECT` rule, as before. Change it with `PATCH /api/v1/app-control/rules/{id}` and a `reason`. The change is audited and reaches hosts like any other rule change.
