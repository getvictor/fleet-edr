...
**Deciding when to promote a rule** is what monitor mode is for, so the evidence sits next to the control. Each rule's row in the admin UI's detection-tuning table carries an **Observed** column: how many times the rule matched in monitor mode over the last 7 days and across how many distinct hosts. The same data is at `GET /api/v1/detection-config/rule-match-counts`, which takes an optional `days` window (default 7, capped at 30, the counter retention window) and reports in `days` the window it actually covered, since a request above the cap is served at the cap. A rule that matched nothing in the window is absent from the response and shows a dash rather than a zero, because a rule may simply have been promoted before the window opened.
