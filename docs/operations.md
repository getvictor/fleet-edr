...
**Per-rule mode** replaces the old whole-rule disable knob. Each rule resolves to `alert`, `monitor` (evaluate but emit an observability log line instead of an alert, to gauge a rule's noise before promoting it), or `disabled` (emit nothing). Absent a setting a rule runs in the mode it declares for itself, which `GET /api/rules` reports as `default_mode`: `alert` for every rule written for this engine, and `monitor` for a rule imported from an upstream corpus, so an unfamiliar rule records what it would have fired on before it is promoted. A setting always overrides that declaration. A disabled rule stays listed in `GET /api/rules` rather than vanishing; the mode an operator set is listed separately at `GET /api/v1/detection-config/rule-settings`. Set it at `PUT /api/v1/detection-config/rule-settings`; an optional `severity_override` rewrites the persisted alert's severity. The `detection_config.read` and `detection_config.write` authz actions gate the surface (admin holds write; admin and senior_analyst hold read).
