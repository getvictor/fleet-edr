...
Pick a release from https://github.com/getvictor/fleet-edr/releases and download the `.pkg` to the target Mac. Example for v0.4.0:
...
Expect: `fleet-edr-v0.4.0.pkg: OK`.
...
pkgutil --check-signature fleet-edr-v0.4.0.pkg
# Expect:
#   Status: signed by a developer certificate issued by Apple for distribution
#   Notarization: trusted by the Apple notary service

# Stapled ticket embedded (works offline)
xcrun stapler validate fleet-edr-v0.4.0.pkg
# Expect: "The validate action worked!"

# Gatekeeper's install assessment
spctl -a -v --type install fleet-edr-v0.4.0.pkg
...
Use the same release tag you downloaded in Step 1 in place of the `v0.4.0` placeholder below; the bundle and the `.pkg` must come from the same release.

```sh
# Install cosign (v3+) if you don't have it: brew install cosign
curl -fLO https://github.com/getvictor/fleet-edr/releases/download/v0.4.0/fleet-edr-v0.4.0.pkg.sigstore.json

cosign verify-blob \
    --bundle fleet-edr-v0.4.0.pkg.sigstore.json \
    --certificate-identity-regexp '^https://github\.com/getvictor/fleet-edr/\.github/workflows/release\.yml@refs/tags/v' \
    --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
    fleet-edr-v0.4.0.pkg
# Expect: "Verified OK"
```
...
```sh
cosign verify ghcr.io/getvictor/fleet-edr-server:v0.4.0 \
    --certificate-identity-regexp '^https://github\.com/getvictor/fleet-edr/\.github/workflows/release\.yml@refs/tags/v' \
    --certificate-oidc-issuer 'https://token.actions.githubusercontent.com'
```
...
```sh
sudo installer -pkg ~/Downloads/fleet-edr-v0.4.0.pkg -target /
```
...
```sh
sudo installer -pkg fleet-edr-v0.4.0.pkg -target /
```
