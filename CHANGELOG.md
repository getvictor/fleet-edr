...
- **Detection no longer slows down as you add servers.** Each server wrote a per-rule statistics row to the database for every batch of events it processed, and because every server writes to the same database that write, rather than the detection work, set the ceiling on how fast the fleet's events could be processed. The statistics are now accumulated in memory and written about every thirty seconds, so processing a batch does no statistics write at all. The numbers in the detection-tuning table are unchanged and, while the database is reachable, stay exact. A database outage or an unclean shutdown can cost the last few seconds of them, and an outage that interrupts a write mid-flight can count that window twice. Per-rule evaluation time is also exported as a metric now, if you would rather read percentiles in your metrics backend than totals in the UI.
