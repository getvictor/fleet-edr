...
- **The process graph loads on hosts with a long history.** On a host carrying millions of process rows the graph failed outright, showing an error instead of the tree, because the server counted every matching row to report how many it was not showing. That count now stops at 10,000 and carries a time budget, so the read always answers; a window with more than that reports "more than 10,000" rather than an exact figure nobody can afford to compute.
- **The event timeline now says when it could not narrow to an alert's chain.** Opening an alert's timeline shows only that alert's processes, but it can only do that for processes carrying the generation identifier it matches on. Where none of them do it lists the whole host, which is why a four-process graph could sit beside a timeline of everything with nothing to explain the difference; where only some do, it lists a partial chain while looking complete; and where the tree could not be read at all, it listed the whole host while the graph beside it reported the error. It now says which of these you are looking at. Which events are listed is unchanged, except that it stops scoping to a chain resolved from a window you are no longer looking at.
