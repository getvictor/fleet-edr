...
- **Rotate any credentials in a proxy URL the agent cannot use.** An agent configured with a proxy scheme it does not speak (anything but `http` or `https`) connected to that host anyway and sent the configured credentials in clear text before discovering it could not talk to it. It now refuses such a proxy at startup, names it in the log, and reaches the server directly. Treat credentials in any non-HTTP proxy URL as exposed and rotate them.
- **Rewrite any `signing_id` exclusion with the team that signed the binary.** A value is now `<TEAMID>:<identifier>` (for example `Q6L2SF6YDW:com.anthropic.claude-code`), or `platform:<identifier>` for a binary Apple ships; `codesign -dv <binary>` prints both. Bare values no longer suppress anything, so those rules fire again until rewritten. An ad-hoc signature can claim any vendor's identifier, so a bare exclusion could be inherited by a planted binary.
...
- **Contain a compromised Mac.** **Contain host** on a host's page cuts the Mac off from the network, except the EDR server and the DHCP and DNS it needs, and **Release host** gives its access back. Open connections are cut, and containment survives restarts and re-enrollment until released. Each change needs `host.isolate` and a reason, is recorded in the audit log, and is made against the state you were shown, so two responders cannot overwrite each other. The host page and host list report whether the Mac has confirmed it, and say so when a Mac is too old to contain or its name filtering cannot be confirmed. Deployment-wide, you can also list extra destinations a contained host may still reach, such as an MDM or forensic server. See [Network containment](docs/operations.md#network-containment).
- **Rules can be browsed and written in the console.** A new **Rules** page lists every rule the deployment runs, with its severity, its mode, and whether it ships with the product or was written here. Operators with `rule_content.write` can create, edit, delete and roll back rules, each change asking for a reason that is audited. A change takes effect within 30 seconds, and a new rule runs in monitor mode until promoted.
- **Application-control rules can run in Detect mode.** A rule with enforcement `DETECT` blocks nothing and keeps a monitor record of each exec it matches that runs, so you can see what a rule would block before promoting it to `PROTECT`. A `DETECT` rule never unblocks an exec another rule blocks. Every new rule asks for one or the other, in the console and the API, with no default. The policy page shows what each Detect rule would have blocked, runs and hosts over the last week, and promotes it from there. See [Detect mode](docs/operations.md#detect-mode).
...
- **Signature exclusions now match a process that was forked and never re-executed.** A process created by `fork` alone runs its parent's image but carried no code-signing record, so every `team_id`, `signing_id` and `cdhash` exclusion silently missed it. Such a process now inherits its parent's signature, and an exec still replaces it. Exclusions that looked ineffective on forking services, `sshd` among them, take effect.
- **Concurrent application-control rule changes no longer fail.** Two operators changing rules in the same policy at once, or one saving while a bulk upsert ran, could have the database abort one of them and return a server error for a change that was valid. Rule changes to a policy now take their turn.
- **The server finishes shutting down in the deployments this product ships.** Every shipped Compose file stopped the server before it had finished draining, so in-flight work and the telemetry describing it were lost on every restart and upgrade. The shipped files now allow the drain to complete.
- **An audit row can no longer go missing for a command that was issued.** The row recording who issued or withdrew a command was written after the change was already saved, so a crash or an audit-store failure in between left the change unrecorded. It is now saved with the change and delivered afterwards, so a slow audit store delays the row rather than the command. A row may arrive late; it is no longer lost.
- **A response action whose outcome was lost no longer stays stuck.** When the connection carrying a command's result died before it arrived, the action sat as delivered with no outcome and nothing asked again. The server now re-asks after a minute and the Mac answers from its own record, without repeating the action, whichever path the Mac is connected by.
- **A single sign-on save can no longer overwrite a change it did not see.** A page left open, or a script that read the settings earlier, silently replaced everything saved since, the issuer, external URL and group mapping included. A save now names the configuration it was editing and is refused if anything changed in the meantime, with nothing written, and the page tells you to reload. On the API the read reports a `version` and a save that returns it is refused with `409`; a save that omits it overwrites as before, so existing automation is unaffected.
- **A single sign-on change no longer lands on a sign-in already under way.** Saving the settings while somebody was signing in could give that operator the role their groups map to under the new settings, not the ones they signed in through. A save now applies from the next sign-in, and never refuses one in flight.
- **Application control no longer offers rule changes an operator cannot make.** An operator with read access and no rule permissions could open any dialog and type an audit reason before being refused. Each control now appears only for an operator holding the permission it needs, and one holding none sees the rules without an actions column.
...
- **Saving an application-control rule without changing it no longer fails.** An edit that sets a rule's fields to the values it already has returned "rule not found". It now succeeds, and since nothing changed it does not push the policy to hosts or add an audit entry.
- **An agent restart no longer reads as a fault in the control-channel telemetry.** A Mac going away, by restart, closed lid or dropped link, ended its control connection the same way a real failure does, so error-rate and latency panels for that connection reported the fleet's uptime rather than its health. An ordinary disconnect is now recorded as a success.
...
- **Upgrade your agents.** Process events are now stamped from the system's own event time, and an agent that loses its control connection reconnects instead of waiting. The server tolerates the old stamps, so an older agent already stops missing most connection detections, but only upgrading gets the control-channel fix.
- **Reboot each Mac after upgrading its agent.** macOS keeps the previous network extension registered until the machine restarts, so network and DNS telemetry stop while process events keep flowing, and the usual health indicators stay green. The sign is a repeating `xpc_bridge_connect failed for group.com.fleetdm.edr.networkextension` in `/var/log/fleet-edr-agent.log`.
- **Each server start now installs the rules that ship with the build.** Rules you wrote and tuning you applied are untouched, but expect the rule list to change on the first start after upgrading.
- **`Suspicious exec chain` gives up one class of detection to make the rest tunable.** Chains whose parent is missing from the host's process tree used to alert naming the parent `(unknown)`, which no exclusion could suppress; they now raise nothing. Chains started by `launchd` still alert and name `/sbin/launchd`, so an exclusion works.
...
- **Destroying sudo policy is now detected.** Emptying `/etc/sudoers` or a fragment under `/etc/sudoers.d/`, and deleting one, both raise a new `Sudoers policy destroyed` alert; neither produced any telemetry before. It is separate from `Sudoers tamper`, which means the opposite: one grants root access, the other takes it away.
- **Detection rules are stored in the database and picked up without a restart.** Each server re-reads within about thirty seconds of a change. While unusable content is stored, a restarting server falls back to the release's rules and can differ from its peers.
- **You can write your own detection rules through the API.** A change that would stop your rules loading is refused, and a check endpoint reports what a change would do without making it. Every change needs a reason and is audited, new rules start in monitor mode, and your rules are credited to your deployment rather than to the community project. Editing requires admin, viewing admin or senior analyst.
- **A rule that is too broad or too slow is caught when you submit it.** A search with no narrowing condition is flagged by name, because a rule resting on one fires constantly and buries the alerts you care about. It warns rather than rejects; a pattern too expensive to match is refused outright.
- **You can roll back a bad rule set.** See which rules were added, removed and changed, and put the previous set back. A rollback sticks across a restart while the next release installs normally, your own rules are kept, and you are told which shipped rules were not restored.
- **The detection-tuning table shows what each rule has been matching and what it costs.** Each row reports matches over the last 7 days, how many hosts they came from, and what the rule's evaluations cost. Read matches as volume rather than alerts, since repeated matches on one process collapse into a single alert. Both honor `EDR_RETENTION_DAYS` and start accumulating on upgrade.
- **A rule that is consistently too slow is dropped on the server that measured it.** One expensive rule used to slow detection for every rule on that server, with nothing to say which. It is now named in the logs and in a counter, and a restart gives it another chance.
- **Host health names each capture provider, and flags one that has stopped silently.** The content filter and the DNS proxy are listed in their own right. One reporting itself running but delivering no events for two hours on an active host shows as degraded, and clears once capture resumes.
- **Two alerts for a sensor that stopped capturing.** `EDR sensor disabled` (High) fires when a provider stops and does not return within 5 seconds; investigate it even once the host reads healthy, because the agent's automatic repair clears the warning without making the stop legitimate. `EDR sensor could not be restored` (Critical) fires once that repair is exhausted, and names the provider to re-enable.
- **A command can be cancelled, and one that waits too long is dropped instead of running late.** A command queued for an unreachable host used to run whenever the host came back, which is a real risk for Kill process since PIDs get reused. Undelivered commands now show as cancelled or expired rather than failed.

### Changed

- **The `dns_c2_beacon` rule is now called "Suspicious process phoning home".** It never detected beaconing: it looks for a program launched from a temporary or world-writable folder that resolves a domain and then connects to the address returned, with no measurement of periodicity. The identifier is unchanged, so exclusions, per-rule settings and historical alerts are unaffected.

### Fixed

- **Network-connection detections no longer miss fast activity on busy hosts.** Process events were stamped up to 0.7 seconds late under load, so a connection could appear to precede the process that made it and nothing fired. Hosts on an older agent improve too.
- **`Suspicious exec chain` now catches payloads it used to miss entirely.** A command run through `zsh -c` was invisible while the same command under `bash` or `sh` alerted, and a payload behind two layered shells was judged by the outermost, often too old for the rule's window.
- **Alerts are no longer dropped while the server is catching up on process data.** A backlogged server could discard a finding instead of retrying it, losing that alert permanently and silently; `Suspicious process phoning home` was the most exposed. Alerts missed before this upgrade are not backfilled.
- **A host whose events cannot be processed no longer stops reporting detections indefinitely.** A batch that failed the same way every time was retried forever, and every rule stopped seeing that host, indistinguishable from one gone quiet. Such a batch is now given up on and counted per host so you can alert on it.
- **Alerts name the right binary for a process that replaced its own image.** A process that spawned something and then ran a different binary could be named by the later one, and a rule looking for a suspicious parent could miss it. Kill process now also refuses a PID that was reused or re-executed.
- **A process's Network tab shows the connection its alert fired on.** Short-lived processes read "No network activity" even when an outbound connection was the reason for the alert. Connections also stop appearing under a different process that shared a PID.
- **Lowering a rule's severity no longer flattens the findings it had singled out.** `DNS beacon to a suspicious domain` rates a finding higher when the domain looks algorithmically generated, and your setting used to overwrite that. It now moves the whole rule and keeps the escalation on top.
- **Detection no longer slows down as you add servers.** Recording what each rule cost set a ceiling on how fast the fleet's events could be processed. That ceiling is gone. Separately, a batch that outlived its lease is no longer finished by two servers at once, which double-counted a rule's matches.
- **`fleet-edr-migrate` applies every MySQL migration, not all but one.** It reported success having skipped one component's tables, so a deployment that migrates as a privileged step and then starts the server without schema-change permission failed on start, with nothing to explain why.
- **Alerts a rule had already found survive a database outage.** When a lookup a rule depends on fails, the retry used to discard whatever that rule had already found, so a long enough outage lost those detections rather than delaying them.
- **Pasting a list of certificate hashes or paths into an application-control policy now works.** The paste-many flow labelled the `CERTIFICATE` and `PATH` rule types "coming soon" and refused any line it inferred as one, though the same rules could be created one at a time.
- **The process graph loads on hosts with a long history.** On a host carrying millions of process rows the graph failed outright, showing an error instead of the tree, because the server counted every matching row to report how many it was not showing. A window matching more than 10,000 now reports "more than 10,000" rather than failing.
- **The event timeline now says when it could not narrow to an alert's chain.** It can narrow only to processes carrying the generation identifier it matches on, so a four-process graph could sit beside a timeline of the whole host with nothing to explain the difference. It now says which of those you are looking at.
- **The ATT&CK coverage page names techniques and groups them by tactic.** It showed most as "Unmapped" with the bare technique id for a name, carrying a hand-maintained list of 12 techniques while the rules covered 65. It now reads from the published ATT&CK data, and the count of techniques covered only by silent rules links to where you tune them.
- **Smaller fixes.** The application control policy list shows how many rules each policy holds, instead of a dash for every one. Exporting a rule you have written over a shipped one returns your file. Submitting a rule reports only what concerns the rule you changed. `Dylib injection via DYLD_INSERT_LIBRARIES` understands options before the assignment, such as `env -i`, and no longer treats unsetting a variable as setting it, while `DYLD injection on exec` stops claiming a shell-assignment form the sensor cannot see. A process tree showing fewer processes than matched now says so. Alerts raised by a community rule before this release are credited to its author. The Mac-free demo's application-control alert cites a policy rule that exists, and its replayed processes carry the generation identifier an alert timeline needs.
