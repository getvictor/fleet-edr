...
- **You can now write your own detection rules through the API, and every change is recorded against whoever made it.** Create, replace and delete rule files under `/api/v1/rule-content/documents`; a rule is checked before it is stored, using the same loader the server loads its rules with, so a rule that is accepted is one your deployment will actually run. `POST /api/v1/rule-content/documents:check` reports what a change would do without making it. Every change requires a reason and appears in the audit log with the document, the reason, and who made it; a rejected change is not recorded, because nothing changed. Editing rules requires the admin role, viewing them admin or senior analyst. A rule you add starts in monitor mode, so it evaluates quietly until you promote it.
