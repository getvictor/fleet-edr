...
- **You can now write your own detection rules through the API, and every change is recorded against whoever made it.** Create, replace and delete rule files under `/api/v1/rule-content/documents`; a rule is checked before it is stored, using the same loader the server loads its rules with, so a change that would stop your rules loading is refused rather than accepted. A rule this sensor cannot run is still stored, with a warning saying so and naming the field to fix, because a rule your deployment will not run is yours to keep. `POST /api/v1/rule-content/documents:check` reports what a change would do without making it. Every change requires a reason and appears in the audit log with the document, the reason, and who made it; a rejected change is not recorded, because nothing changed. Editing rules requires the admin role, viewing them admin or senior analyst. A rule you add starts in monitor mode, so it evaluates quietly until you promote it.
- **A detection rule that is consistently too slow is now dropped on the server that measured it, and reported.** Until now one expensive rule slowed detection for every rule on that server, with nothing to say which one. A rule that repeatedly takes longer than its budget stops being evaluated there, the rule is named in the logs and in a counter with what it measured, and every other rule keeps running. The rule's configured mode is left alone, since this is the server protecting itself rather than a setting you made; a restart gives the rule another chance. Nothing shipped with this release comes close to the budget.
...
- **Alerts raised by a community rule before this release now show who wrote it.** Detections imported from the community carry an author credit, and alerts have shown it since the feature shipped; alerts raised before then showed none. Those are credited on the next server start. Alerts from Fleet EDR's own rules are not backfilled, so a blank credit keeps meaning "raised before this release" rather than becoming a claim about authorship, and application-control blocks are left alone because the rule named there is your own policy entry rather than a detection anyone here authored.

- **`DYLD injection on exec` no longer claims to catch a shape it cannot see.** The rule said it detected both an `env DYLD_INSERT_LIBRARIES=... prog` invocation and a shell assignment such as `DYLD_INSERT_LIBRARIES=... /bin/ls`. Only the first was ever true: the sensor records a process's arguments, and a shell applies those variables without passing them as arguments, so that command reaches the server with the assignment absent entirely. Measured across 670,185 real exec events, the shell shape appeared zero times. Nothing you were detecting stops being detected; what changes is that the rule's description, and the coverage you would have credited it with, now match what it does. Detecting the shell form needs the agent to capture the environment, which is tracked separately.
- **Detection no longer slows down as you add servers.** Recording what each rule cost used to happen once per batch of events, against the database every server shares, so it set the ceiling on how fast the fleet's events could be processed rather than the detection work doing so. That ceiling is gone. The numbers in the detection-tuning table are unchanged, though a brief database outage or an unclean stop can now cost you the last few seconds of them.

- **Per-rule evaluation time is now available as a metric.** If you collect metrics, you can read percentiles for how long each rule takes rather than the totals the detection-tuning table shows, which is the difference between seeing that a rule is slow on average and seeing that it is occasionally very slow.
...
- **A batch of events that takes unusually long to process is no longer counted twice.** Work is leased to one server for five minutes; if processing outlived that lease the work was offered to another server, and both would then report it complete with neither noticing. Alerts were never duplicated, but the per-rule match counts behind the detection-tuning table could be, making a rule look noisier than it is. Whichever server still holds the work now completes it, the other stops and says so in the logs, which is also the first sign you get that processing is running past its lease.
