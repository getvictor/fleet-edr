...
- **Rotate any credentials in a proxy URL the agent cannot use.** An agent configured with a proxy scheme it does not speak (anything but `http` or `https`) connected to that host anyway and sent the credentials in clear text first. It now refuses such a proxy at startup and reaches the server directly. Treat those credentials as exposed and rotate them.
...
- **Application-control API clients must name each rule's enforcement.** Creating a rule, and each item of a bulk upsert, now requires `enforcement`: `PROTECT` to block or `DETECT` to record without blocking. Add `"enforcement": "PROTECT"` to existing automation to keep its rules blocking.
- **A third profile keeps users from turning off the agent.** `edr-login-items.mobileconfig` marks the Fleet EDR background items as managed, so they cannot be switched off in Login Items & Extensions (macOS 13 and later). Push it through your MDM with the other two profiles.
- **Alerts are now deleted 180 days after their last triage activity.** Nothing deleted them before. Set `EDR_ALERT_RETENTION_DAYS` (0 keeps them forever) before upgrading if you want them kept longer: deletion starts as soon as the upgraded server does, then repeats hourly.
...
- **Application-control rules can run in Detect mode.** A `DETECT` rule blocks nothing and records each exec it matches, so you can see what a rule would block before promoting it to `PROTECT`, and it never unblocks an exec another rule blocks. Every new rule asks for one or the other, with no default. The policy page shows what each Detect rule would have blocked over the last week and promotes it from there. See [Detect mode](docs/operations.md#detect-mode).
...
- **Signature exclusions now match a process that was forked and never re-executed.** Such a process runs its parent's image but carried no code-signing record, so every `team_id`, `signing_id` and `cdhash` exclusion silently missed it. Exclusions that looked ineffective on forking services, `sshd` among them, take effect.
- **A single sign-on save can no longer overwrite a change it did not see.** A page left open, or a script that read the settings earlier, silently replaced everything saved since. A save now names the configuration it was editing and is refused if anything changed, with nothing written. On the API the read reports a `version` and a save that returns it is refused with `409`; omitting it overwrites as before, so existing automation is unaffected.
- **A single sign-on change no longer lands on a sign-in already under way.** Saving while somebody was signing in could give that operator the role their groups map to under the new settings. A save now applies from the next sign-in.
- **Application control no longer offers rule changes an operator cannot make.** An operator with read access and no rule permissions could open any dialog and type an audit reason before being refused.
- **Concurrent application-control rule changes no longer fail.** Two operators changing rules in one policy at once could have the database abort one and return a server error for a change that was valid.
- **The server finishes shutting down in the deployments this product ships.** Every shipped Compose file stopped it before the drain completed, losing in-flight work on every restart and upgrade.
- **An audit row can no longer go missing for a command that was issued.** The row was written after the change was saved, so a crash in between left it unrecorded. It is now saved with the change and delivered afterwards; it may arrive late, but it is not lost.
- **A response action whose outcome was lost no longer stays stuck.** The server re-asks after a minute and the Mac answers from its own record, without repeating the action.
- **A Mac that needs a restart after an agent upgrade says so**, instead of reporting its network extension as not activated.
- **A renamed remote-access tool is detected by its signature.** A shipped rule catching MeshAgent renamed to hide it could not run; the compiled-in name is now read from the code signature, which a rename cannot change. 67 of the 69 shipped rules run, up from 66.
- **An application-control alert links to the policy that blocked**, so an analyst can reach the rule that denied the executable.
