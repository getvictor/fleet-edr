...
- **Rules can be browsed and written in the console.** A new **Rules** page lists every rule the deployment runs, with its severity, the mode it runs in, and whether it ships with the product or was written on this deployment, and a rule loaded from a stored document shows that document as written. Operators with `rule_content.write` can create a rule, check it against the loader before saving, edit or delete their own rules, and roll back the shipped rules, each with a reason for the audit log. A change takes effect when the server next reloads its rules, within 30 seconds, and a new rule runs in monitor mode until promoted.
