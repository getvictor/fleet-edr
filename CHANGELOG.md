...
- **A single sign-on save can no longer overwrite a change it did not see.** `PUT /api/settings/sso` replaces the whole configuration, so a client that read the settings earlier silently overwrote everything saved since, the issuer, external URL and group mapping included. The read now reports a `version`, and a save that sends it back is refused with `409` if anything changed in the meantime, having written nothing. A save that omits it overwrites as before, so existing automation keeps working unchanged.
