...
- **A problem reading a host's process history no longer costs you the detections that depended on it.** When that read failed, the rules needing it were treated as though they were broken: the events they were judging were marked processed and never looked at again, so anything those rules would have caught was lost, with only a debug line to show for it. Those events are now retried until the read succeeds, and a failed read is reported plainly rather than logged as a routine wait. If the problem persists the events are set aside so the host keeps processing, and the record names the host and the failure. No action is required; events processed during an outage before this upgrade are not re-evaluated.
