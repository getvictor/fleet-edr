...
- **`Suspicious exec chain` no longer raises alerts naming an unknown parent, which no exclusion could silence.** When the process that started the shell was missing from the host's recorded process tree, the alert named the parent as `(unknown)`. Parent exclusions match on the parent's path, so if one of these was noisy on your fleet there was no way to suppress it short of turning the rule off. Those chains now raise nothing at all, so this is a detection the rule gives up in exchange for the ones you can tune: an alert nobody can silence tends to get the whole rule turned off. If you added a parent exclusion that never seemed to take effect, this is why.
