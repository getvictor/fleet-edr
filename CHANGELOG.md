...
- **Detection rules are now refused if their patterns would be too expensive to match.** This matters ahead of rule authoring: a pattern is matched against every event a host sends, so one written carelessly can slow detection for the whole fleet. A rule is refused when it loads if one of its patterns costs too much to match, or if a single field's patterns cost too much together, and the refusal names the field and the limit so it can be corrected. The second half matters as much as the first: a field's patterns are tried until one matches, so a long list of individually reasonable patterns is what an event actually pays for. Nothing you run today is affected, since every rule shipped with this release is far inside both limits; they exist for rules written from now on.
