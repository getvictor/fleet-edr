...
- **Rewrite any `signing_id` exclusion with the team that signed the binary.** A `signing_id` exclusion value is now `<TEAMID>:<identifier>` (for example `Q6L2SF6YDW:com.anthropic.claude-code`), or `platform:<identifier>` for a binary Apple ships. Existing bare values no longer suppress anything, so those rules fire again until you rewrite them; `codesign -dv <binary>` prints both fields. The identifier on its own is whatever the signer typed, and an ad-hoc signature can claim any vendor's, so a bare exclusion could be inherited by a planted binary.
...
- **Contain a compromised Mac.** **Contain host** on a host's page cuts the Mac off from the network, except the EDR server (or the proxy the agent reaches it through) and the DHCP and DNS it needs, and **Release host** gives its access back. Open connections are cut, DNS goes only to the Mac's own resolvers, and while the EDR's DNS proxy is running only the server's name resolves. The Mac keeps containment through restarts until released, including an agent that has to enroll again while contained. Each change asks for a reason, recorded in the audit log, and needs `host.isolate`, plus a recent sign-in when an operator makes it. The host page and host list show whether the Mac has confirmed it. A Mac on an earlier agent reports Containment failed. If the Mac reports its DNS proxy switched off, the host page says its DNS is restricted by destination only; if no DNS capture is reaching the server from a Mac that is otherwise reporting, it says name filtering is unconfirmed. See [Network containment](docs/operations.md#network-containment).
- **Rules can be browsed and written in the console.** A new **Rules** page lists every rule the deployment runs, with its severity, the mode it runs in, and whether it ships with the product or was written on this deployment, and a rule loaded from a stored document shows that document as written. Operators with `rule_content.write` can create a rule once it passes a check against the loader, edit or delete their own rules, and roll back the shipped rules; each of those changes asks for a reason, recorded in the audit log. A change takes effect when the server next reloads its rules, within 30 seconds, and a new rule runs in monitor mode until promoted.
...
- **Two operators can no longer overwrite each other's containment decision.** Containing or releasing a host is now made against the state you were shown: if someone else changed the host while you were deciding, your change is refused and the page shows you what the host holds now, instead of quietly applying over their decision. Automation can ask for the same by sending the version it read.
...
- **An audit row can no longer go missing for a change that happened.** Containing or releasing a host, and issuing or withdrawing a command, recorded who did it only after the change had already been saved, so an audit-store failure or a crash in between left the change with nothing recording it. The row is now saved with the change itself and delivered afterwards, so the action no longer waits for the audit log to be written: an audit store that is slow or catching up after an outage no longer slows down containing a host or issuing a command. A row may arrive late after an outage, but it is no longer lost.
- **A response action whose outcome was lost no longer stays stuck.** When the connection carrying a command's outcome died before the outcome reached the server, the action stayed in place as delivered with no result, and nothing asked the Mac again. The server now asks the Mac once a minute has passed, and the Mac answers from its own record of what it did, without repeating the action. A Mac without that connection now asks the same question itself, so the recovery no longer depends on which path the Mac is using.
...
- **A sign-in is judged under the single sign-on configuration that verified it.** The configuration was read once to verify the token and again to decide the operator's role, so an administrator saving in between could have a sign-in verified under the old provider and its group mapping applied from the new one. Both now come from one read. A save applies from the next sign-in, and never refuses one already in flight.
- **A single sign-on save can no longer overwrite a change it did not see.** Saving the Single sign-on settings replaced the whole configuration, so a page left open, or a script that read the settings earlier, silently overwrote everything saved since: the issuer, external URL and group mapping included. A save now names the configuration it was editing and is refused if anything changed in the meantime, with nothing written, and the page says so and tells you to reload rather than showing an error code. On the API, the read reports a `version` and a save that sends it back is refused with `409`; a save that omits it overwrites as before, so existing automation keeps working unchanged.
- **Application control no longer offers rule changes an operator cannot make.** The policy page showed Add rule, Paste many, and each rule's Promote, Edit, Disable and Delete to anyone who could open it, so an operator with read access and no rule permissions could fill in a dialog and an audit reason before being refused. Each control now appears only for an operator who holds the permission it needs, and one who holds none sees the rules without an actions column.
