...
- **Detection rules are now stored in the database, and the server picks up a change without a restart.** The rules a deployment runs were fixed at build time, so changing them meant shipping a new binary. The rule content now lives in the database, seeded on first start from the set built into the release, and each server re-reads it within about thirty seconds of a change. Nothing to do on upgrade: the first start seeds the content and the rules that run are the same ones as before. If the database cannot be reached, the rules already running stay in force and the server keeps retrying rather than replacing them. If the stored content is readable but yields no working rules, whether it is empty, malformed, or made entirely of rules this sensor cannot run, every server falls back to the release's own rules: expect a bad publish to revert what the fleet detects rather than to leave each server on whatever it happened to be running. Editing the content directly is not yet supported; the API for publishing rule packs follows.
