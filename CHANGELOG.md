...
- **Alerts from a shell that launchd started can now be silenced without turning the rule off.** `Suspicious execution from shell` and `Shell established outbound connection` name the process that started the shell, and for a shell launchd started directly they had nothing to name, so a parent exclusion could not match one and the only way to stop a recurring alert of this shape was disabling the rule. Those alerts now name `/sbin/launchd`, which is what started the shell, and a parent-path-glob exclusion for it works. Note that such an exclusion silences EVERY launchd-started shell chain for that rule, real persistence execution included, so prefer a narrower exclusion where one fits. Existing alerts are not re-evaluated.
