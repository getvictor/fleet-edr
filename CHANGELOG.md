...
- **Alerts raised by a community rule before this release now show who wrote it.** Detections imported from the community carry an author credit, and alerts have shown it since the feature shipped, but alerts raised before then displayed no credit at all. Those are credited on the next server start. Alerts from rules Fleet EDR wrote are deliberately left blank, so you can still tell them apart from alerts that predate the credit, and application-control blocks are left alone because the rule named there is your own policy entry rather than a detection anyone here authored.
