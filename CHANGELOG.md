...
- **Detection rules are now refused if their patterns would be too expensive to match.** This matters ahead of rule authoring: a pattern is matched against every event a host sends, so one written carelessly can slow detection for the whole fleet. Three limits are checked when a rule loads, and a refusal names the field and the limit so it can be corrected: a run of single-character `?` wildcards between two `*`, the length of a regular expression, and how many values one field may list. Nothing you run today is affected, since every rule shipped with this release is far inside all three; the limits exist for rules written from now on.
