...
- **Detection rules are now stored in the database, and the server picks up a change without a restart.** The rules a deployment runs were fixed at build time, so changing them meant shipping a new binary. The rule content now lives in the database, seeded on first start from the set built into the release, and each server re-reads it within about thirty seconds of a change. Nothing to do on upgrade: the first start seeds the content and the rules that run are the same ones as before. If the database is unreachable or the stored content cannot be read, the rules already running stay in force rather than being replaced, and the release's own content is used when a server starts with nothing loaded. Editing the content directly is not yet supported; the API for publishing rule packs follows.
