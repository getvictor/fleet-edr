...
describe("service-account API client", () => {
  it("listServiceAccounts GETs the collection and unwraps the envelope", async () => {
    const mock = stubFetch({ service_accounts: [{ id: 1, client_id: "sa_a", name: "a", role: "analyst", status: "active", created_at: "", expires_at: "" }] });
  ...
  it("createServiceAccount POSTs the body with the CSRF header attached", async () => {
    ...
    const mock = stubFetch({ id: 2, client_id: "sa_b", name: "b", role: "analyst", status: "active", created_at: "", expires_at: "", secret: "edrsa_x" });
